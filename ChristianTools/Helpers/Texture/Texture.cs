    ...
    public partial class Texture
    {
        public static Texture2D[] SliceHorizontalTexture(Texture2D originalTexture, int cuts)
        {
            ...
            for (int i = 0; i < cuts; i++)
            {
                ...
                textures[i] = Helpers.Texture.CropTexture(originalTexture, rectangle);
            }
        ...
        private static Dictionary<int, Texture2D> GetTileTextures(Texture2D atlasTexture, int pixelsPerTile_Width, int pixelsPerTile_Height, int units_Width, int units_Height, int scaleFactor)
        {
            ...
            for (int y = 0; y < (pixelsPerTile_Height * units_Height); y += pixelsPerTile_Height)
            {
                for (int x = 0; x < (pixelsPerTile_Width * units_Width); x += pixelsPerTile_Width)
                {
                    ...
                    Texture2D tileTexture_Scaled = ChristianTools.Helpers.Texture.CropTexture(
        ...
        public static Texture2D ScaleTexture(Texture2D originalTexture, int scaleFactor)
        {
            ...
            Color[,] multidimentionalColors = ChristianTools.Helpers.Other.ToMultidimentional<Color>(originalColors, originalTexture.Width, originalTexture.Height);

            Color[,] expandedColors = ChristianTools.Helpers.Other.Expand<Color>(multidimentionalColors, scaleFactor);

            Color[] flatResult = ChristianTools.Helpers.Other.FlattenArray<Color>(expandedColors);
        ...
        public static Texture2D GetTexture(string imageName, int scaleFactor)
        {
            ...
            Texture2D texture2D = ChristianTools.Helpers.Texture.GetTexture(imageName);
            Texture2D result = ChristianTools.Helpers.Texture.ScaleTexture(texture2D, scaleFactor);
            return result;
        }
        ...
        public static Texture2D CropTexture(Texture2D originalTexture, Rectangle extractRectangle, int scaleFactor = 1)
        {
            ...
            if (scaleFactor > 1)
                subtexture = ChristianTools.Helpers.Texture.ScaleTexture(subtexture, scaleFactor);
        ...
        public static Texture2D CreateTriangle(Color color, int Width, int Height, PointDirection pointDirection)
        {
            ...
            float m1 = ChristianTools.Helpers.MyMath.M(p1.ToVector2(), p2.ToVector2());
            float m2 = ChristianTools.Helpers.MyMath.M(p3.ToVector2(), p2.ToVector2());
            ...
            switch (pointDirection)
            {
                ...
                {
                    Color[,] multidimentionalColors = ChristianTools.Helpers.Other.ToMultidimentional(colors, Width, Height);
                    multidimentionalColors = ChristianTools.Helpers.Other.RotateArray_90_AntiClockwise(multidimentionalColors);
                    colors = ChristianTools.Helpers.Other.FlattenArray(multidimentionalColors);
                }
                    break;
                case PointDirection.Down:
                {
                    Color[,] multidimentionalColors = ChristianTools.Helpers.Other.ToMultidimentional(colors, Width, Height);
                    multidimentionalColors = ChristianTools.Helpers.Other.RotateArray_270_AntiClockwise(multidimentionalColors);
                    colors = ChristianTools.Helpers.Other.FlattenArray(multidimentionalColors);
                }
                    break;
                ...
                {
                    Color[,] multidimentionalColors = ChristianTools.Helpers.Other.ToMultidimentional(colors, Width, Height);
                    multidimentionalColors = ChristianTools.Helpers.Other.RotateArray_180_AntiClockwise(multidimentionalColors);
                    colors = ChristianTools.Helpers.Other.FlattenArray(multidimentionalColors);
                }
                    break;
            }
