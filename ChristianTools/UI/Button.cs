...
namespace Showroom_CrossPlatform.UI
{
    public class Button : IUI
    {
        ...
        public Button(Rectangle rectangle, string text, string tag, DxOnClickAction OnClickAction, Texture2D defaultTexture = null,  Texture2D mouseOverTexture = null)
        {
            this.rectangle = rectangle;
            this.defaultTexture = defaultTexture ?? ChristianTools.Helpers.MyColors.CreateColorTexture(Color.LightGray);
            this.mouseOverTexture = mouseOverTexture ?? ChristianTools.Helpers.MyColors.CreateColorTexture(Color.Gray);
        ...
        private void UpdateSystem(InputState lastInputState, InputState inputState, IScene scene)
        {
            ...
            if (scene.camera != null)
                tempRectangle = new Rectangle((int)(rectangle.X + scene.camera.rectangle.X), (int)(rectangle.Y + scene.camera.rectangle.Y), rectangle.Width, rectangle.Height);
            else
