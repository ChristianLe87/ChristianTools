...
namespace Showroom_CrossPlatform.UI
{
	public class Label : IUI
	{
		...
		public Label(Rectangle rectangle, string text, TextAlignment textAlignment, Texture2D texture= null, string tag="")
		{
		...
		public void DrawSystem(SpriteBatch spriteBatch, IScene scene)
		{
			...
			{
				rec = new Rectangle(rectangle.Location + new Point(scene.camera.rectangle.X, scene.camera.rectangle.Y), rectangle.Size);
			...
			spriteBatch.DrawString(ChristianGame.spriteFont, text, (GetTextPosition(ChristianGame.spriteFont) + p).ToVector2(), Color.White);
		}
